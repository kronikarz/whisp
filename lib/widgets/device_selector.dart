...
class _DeviceSelectorState extends State<DeviceSelector> {
  List<AudioDevice> _audioDevices = <AudioDevice>[];
      ...
      child: Column(
        ...
        children: <Widget>[
          SizedBox(
            ...
            child: Column(
              children: <Widget>[
                if (_audioDevices.isNotEmpty) ...<Widget>[
                  Center(
                    child: Text(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                return ListTile(
                  ...
                  leading: (_audioIcons.containsKey(_audioDevices[index].id))
                      ? Image.memory(
                          _audioIcons[_audioDevices[index].id] ?? Uint8List(0),
                          width: 28,
                          height: 28,
                  ...
                  title: Text(
                    _audioDevices[index].name,
                    style: const TextStyle(fontSize: 12),
                  ),
                  trailing: IconButton(
                    icon: Icon(_audioDevices[index].isActive == true ? Icons.radio_button_checked : Icons.radio_button_off),
                    iconSize: 16,
                    onPressed: () async {
                      await Audio.setDefaultDevice(_audioDevices[index].id);
  ...
  Future<void> _reloadAudioDevices() async {
    ...
    _audioDevices = await Audio.enumDevices(AudioDeviceType.input) ?? <AudioDevice>[];
    _volume = await Audio.getVolume(AudioDeviceType.input);

    _audioIcons = <String, Uint8List>{};
    for (AudioDevice audioDevice in _audioDevices) {
